...
import { getJWT } from '../models/sharedConfig';
...
export async function add_subject_end(req: Request, res: Response) {
    ...
    if(sub_verification(req,res)){
        ...
        if(subject){
            ...
            if("iss" in subject )
            {
                ...
                await findSubjectDB_2(subject.format,subject.iss,subject.value).then((found) => {
                    if (!found) {
                        return res.status(403).json({error: "Failed to add subject, not found in database"})                
                    }
                    else
                    {
                        add_subject(client_token.sub, subject.value, "enabled");
                        if(req.body.subject.format !== "email")
                        {
                            return res.status(200).json({Warning: "Monokee will recognize only subjects with format 'email' "});
                        }
                        else
        ...
        //If the ExstractSubjectId returns undefined we don't have a subject that follows our standard 
        else{
