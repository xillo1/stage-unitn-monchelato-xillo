...
import {getJWT, get_config, update_status_config} from '../models/sharedConfig';
...
export function getStatus(req: Request, res: Response) {
    ...
    if (subject_claim !== undefined) {
        ...
        if(subject){
            ...
            const found = get_subject(client_token.sub, subject.value);
            if(found !== undefined){
    ...
    //If there is no subject, we send the status of the stream
        ...
        let config = get_config(client_token.sub);
        res.setHeader('Content-Type', 'application/json');
    ...
    try{
        ...
        if (req.body.hasOwnProperty('subject') && req.body.subject !== undefined) {
            ...
            if(subject){
                const found = get_subject(client_token.sub, subject.value);                
                if(found !== undefined){
                    const result = update_status(client_token.sub, subject.value, newState);
                    if(result)
                    {
                        res.setHeader('Content-Type', 'application/json');
                        res.status(200).json({
        ...
        //Else we update the status of the stream
            ...
            update_status_config(client_token.sub, newState);
